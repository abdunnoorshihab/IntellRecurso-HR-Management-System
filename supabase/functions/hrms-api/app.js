...
const ROLE_ACCESS = {
  ...
  chairman: { view: ACCESS_MODULES, create: ['employees','organization','attendance','tasks','events','leave','performance','reports','requisitions','conveyance','salary','funds','letters','administration'], edit: ['employees','organization','attendance','tasks','events','leave','performance','reports','requisitions','conveyance','salary','funds','letters','administration'], delete: [], approve: ['leave','requisitions','conveyance','funds'] },
...
app.get('/api/employees', requireAuth, allowAccess('employees', 'view', 'admin','hr','ceo','chairman'), async (req, res) => {
  try {
...
app.get('/api/admin/users', requireAuth, allow('admin','hr','ceo','chairman'), async (req,res)=>{try{const items=await db.all('SELECT u.id,u.employee_id,u.email,u.role,u.status,u.created_at,e.name employee_name FROM users u LEFT JOIN employees e ON e.id=u.employee_id ORDER BY u.id');res.json({items});}catch(e){sendDbError(res,e);}});
app.get('/api/admin/access/:userId', requireAuth, allow('admin','hr','ceo','chairman'), async (req,res)=>{try{const userId=Number(req.params.userId);res.json({items:await db.all('SELECT module,can_view,can_create,can_edit,can_delete,can_approve,data_scope FROM user_access WHERE user_id=? ORDER BY module',userId)});}catch(e){sendDbError(res,e);}});
app.put('/api/admin/access/:userId', requireAuth, allow('admin','hr','ceo','chairman'), async (req,res)=>{try{const userId=Number(req.params.userId),permissions=Array.isArray(req.body.permissions)?req.body.permissions:[];const validActions=['can_view','can_create','can_edit','can_delete','can_approve'],validScopes=['self','team','all'];await db.run('DELETE FROM user_access WHERE user_id=?',userId);for(const item of permissions){if(!ACCESS_MODULES.includes(item.module))continue;const values=validActions.map(key=>Boolean(item[key]));const scope=validScopes.includes(item.data_scope)?item.data_scope:'self';await db.run('INSERT INTO user_access(user_id,module,can_view,can_create,can_edit,can_delete,can_approve,data_scope,updated_at) VALUES(?,?,?,?,?,?,?,?,?)',userId,item.module,...values,scope,now());}await audit(req.user.id,'update','user_access',userId,{modules:permissions.length});res.json({ok:true});}catch(e){sendDbError(res,e);}});
app.post('/api/admin/users', requireAuth, allow('admin','hr','ceo','chairman'), async (req,res)=>{try{const email=cleanText(req.body.email,'').toLowerCase(),password=String(req.body.password||''),role=cleanText(req.body.role,'employee');if(!email||password.length<10)return res.status(400).json({error:'Valid email and password of at least 10 characters are required'});if(!ROLES.includes(role))return res.status(400).json({error:'Invalid role'});const r=await db.get('INSERT INTO users(employee_id,email,password_hash,role,status,created_at) VALUES(?,?,?,?,?,?) RETURNING id',req.body.employee_id?Number(req.body.employee_id):null,email,hashPassword(password),role,'active',now());await audit(req.user.id,'create','user',r.id,{email,role});res.status(201).json({id:Number(r.id)});}catch(e){sendDbError(res,e);}});
app.patch('/api/admin/users/:id', requireAuth, allow('admin','hr','ceo','chairman'), async (req,res)=>{try{const id=Number(req.params.id),old=await db.get('SELECT * FROM users WHERE id=?',id);if(!old)return res.status(404).json({error:'User not found'});const role=cleanText(req.body.role,old.role),status=cleanText(req.body.status,old.status),email=cleanText(req.body.email,old.email).toLowerCase();if(!ROLES.includes(role))return res.status(400).json({error:'Invalid role'});const cols=['email=?','role=?','status=?'],vals=[email,role,status];if(req.body.password){if(String(req.body.password).length<10)return res.status(400).json({error:'Password must be at least 10 characters'});cols.push('password_hash=?');vals.push(hashPassword(String(req.body.password)));}vals.push(id);await db.run(`UPDATE users SET ${cols.join(',')} WHERE id=?`,...vals);await audit(req.user.id,'update','user',id,{email,role,status});res.json({ok:true});}catch(e){sendDbError(res,e);}});
app.get('/api/admin/settings', requireAuth, allow('admin','hr','ceo','chairman'), async (req,res)=>{try{res.json({items:await db.all('SELECT * FROM settings ORDER BY key')});}catch(e){sendDbError(res,e);}});
app.put('/api/admin/settings/:key', requireAuth, allow('admin','hr','ceo','chairman'), async (req,res)=>{try{const key=cleanText(req.params.key,''),value=cleanText(req.body.value,'');await db.run('INSERT INTO settings(key,value,updated_at) VALUES(?,?,?) ON CONFLICT(key) DO UPDATE SET value=EXCLUDED.value,updated_at=EXCLUDED.updated_at',key,value,now());await audit(req.user.id,'update','setting',key,{value});res.json({ok:true});}catch(e){sendDbError(res,e);}});
app.get('/api/admin/audit', requireAuth, allow('admin','hr','ceo','chairman'), async (req,res)=>{try{const items=await db.all('SELECT a.*,u.email user_email FROM audit_logs a LEFT JOIN users u ON u.id=a.user_id ORDER BY a.id DESC LIMIT 200');res.json({items});}catch(e){sendDbError(res,e);}});
